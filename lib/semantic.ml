...
let check_main_function (prog : comp_unit) =
  let found =
    List.exists (fun f -> f.name = "main" && f.params = [] && f.ret_type = Int) prog
  in
  if not found then raise (Semantic_error "必须有一个名为 main、无参数且返回 int 的主函数")
;;
...
let check_var_assign name expr_typ symtab =
  match Symbol.find_symbol name symtab with
  | Some (Var { typ; _ }) ->
    if typ <> expr_typ then raise (Semantic_error ("赋值类型不匹配：" ^ name))
...
let check_in_loop ctx keyword =
  if ctx.in_loop = 0 then raise (Semantic_error (keyword ^ " 语句只能出现在循环内部"))
;;
...
let rec infer_expr (symtab : Symbol.t) (ctx : context) (expr : Ast.expr) : Ast.return_type
  ...
  | BinaryOp (op, e1, e2) ->
    ...
    (match op with
     | Add | Sub | Mul | Div | Mod | Eq | Neq | Lt | Leq | Gt | Geq | And | Or ->
       if t1 <> Int || t2 <> Int then raise (Semantic_error "二元运算符操作数类型必须为 int");
       if op = Div || op = Mod
       then (
         match e2 with
         | Int 0 -> raise (Semantic_error "除数不能为零")
         | _ -> ());
       Int)
  | Call (fname, args) ->
    (match require_declared fname symtab with
     | Func { ret_type; params; _ } ->
       ...
       then raise (Semantic_error ("函数参数数量不匹配：" ^ fname));
       ignore
         (List.map2
            (fun pt at -> if pt <> at then raise (Semantic_error ("函数参数类型不匹配：" ^ fname)))
            param_types
            arg_types);
       ret_type
...
let rec check_stmt symtab ctx stmt =
  ...
  | Empty -> symtab, false
  | Expr e ->
    ignore (infer_expr symtab ctx e);
    symtab, false
  | Block stmts ->
  ...
  | If (cond, s1, None) ->
    let t = infer_expr symtab ctx cond in
    if t <> Int then raise (Semantic_error "if 条件类型必须为 int");
    ignore (check_stmt symtab ctx s1);
    symtab, false
  | While (cond, body) ->
    let t = infer_expr symtab ctx cond in
    ...
    if always_true && body_ret then symtab, true else symtab, false
  | Break ->
    check_in_loop ctx "break";
    symtab, false
  | Continue ->
    check_in_loop ctx "continue";
    symtab, false
  | Declare (name, e) ->
    let typ = infer_expr symtab ctx e in
    if Symbol.is_declared_in_current_scope name symtab
    then raise (Semantic_error ("变量重复声明：" ^ name));
    let symtab' =
      Symbol.add_symbol name (Var { typ; declared = true; offset = 0 }) symtab
    in
    symtab', false
  | Assign (name, e) ->
    let typ = infer_expr symtab ctx e in
...
let check_func symtab fdef =
  ...
  let symtab'' =
    List.fold_left
      (fun s p ->
         match p with
         | Param name ->
           if Symbol.is_declared_in_current_scope name s
           then raise (Semantic_error ("参数名重复：" ^ name));
  ...
  (* int 函数所有路径必须 return *)
  if fdef.ret_type = Int && not has_return
  then raise (NotAllPathsReturn ("int 类型函数 " ^ fdef.name ^ " 不是所有路径都返回值"))
;;
...
let analyze_program (prog : comp_unit) =
  ...
  let symtab =
    List.fold_left
      (fun s f ->
         if Symbol.is_declared_in_current_scope f.name s
         then raise (Semantic_error ("重复定义的函数名：" ^ f.name));
         Symbol.add_symbol
           f.name
           (Func
              { ret_type = f.ret_type
              ; params = params_to_typ_name_list f.params
              ; declared = true
              })
           s)
      symtab
